...
class QuarkusPetclinicAWSLambdaJVMColdStart extends Simulation {
  ...
  val quarkusPetclinicJVM: ScenarioBuilder = scenario("AWS Lambda - Quarkus Petclinic JVM - Cold Start Testing")
    .repeat(400, "n") {
      exec(
        OwnerAPI.getOwners
      )
