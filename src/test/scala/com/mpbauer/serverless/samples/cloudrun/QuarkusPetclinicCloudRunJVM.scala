...
class QuarkusPetclinicCloudRunJVM extends Simulation {

  val httpProtocol: HttpProtocolBuilder = http
    .baseUrl("https://quarkus-petclinic-rest-jvm-dev-s2xflp6dzq-ey.a.run.app")
    .inferHtmlResources()
  ...
  val quarkusPetclinicJVM: ScenarioBuilder = scenario("Google Cloud Run - Quarkus Petclinic JVM")
    .feed(jsonFileFeeder)
    .repeat(500, "n") {
      exec(
  ...
  setUp(
    quarkusPetclinicJVM.inject(rampUsers(70).during(14.seconds)).protocols(httpProtocol)
  )
}